...
#[test]
fn injection_script_moves_export_and_project_move_into_header_menu() {
    let script = assets::injection_script(57321).replace("\r\n", "\n");

    assert!(script.contains("function installTopSessionMenuActions()"));
    assert!(script.contains("function injectTopSessionMenuItems("));
    assert!(script.contains("function topSessionMenuActionRef()"));
    assert!(script.contains("data-codex-top-session-menu-item"));
    assert!(script.contains("codex-top-session-menu-item"));
    assert!(script.contains("导出 Markdown"));
    assert!(script.contains("移动到项目"));
    assert!(script.contains("openTopSessionFallbackMenu(anchor)"));
